...
/// Test that when using --bin flag, only the specified binary and its dependencies
/// are included in the recipe, not other workspace members.
/// See https://github.com/LukeMathWalker/cargo-chef/issues/XXX
#[test]
pub fn workspace_bin_isolates_dependencies() {
    // Arrange
    // We create a workspace with two packages: bar and foo
    // - bar: a binary package with no dependencies
    // - foo: a binary package that depends on either
    // When we run `cargo chef prepare --bin bar`, foo should NOT be in the recipe
    let project = CargoWorkspace::new()
        .manifest(
            ".",
            r#"
...
"#,
    ...
    // Act: Prepare recipe for bar only
    let bar_skeleton = Skeleton::derive(project.path(), Some("bar".into())).unwrap();

    // Assert: barSkeleton should only contain bar, not foo
    let manifest_paths: Vec<_> = bar_skeleton
        .manifests
        .iter()
        .map(|m| m.relative_path.to_string_lossy().to_string())
        .collect();

    // Should contain root Cargo.toml and bar/Cargo.toml, but NOT foo/Cargo.toml
    assert!(
        manifest_paths.contains(&"Cargo.toml".to_string()),
        "Root Cargo.toml should be present"
    );
    assert!(
        manifest_paths.contains(&"bar/Cargo.toml".to_string()),
        "bar/Cargo.toml should be present"
    );
    assert!(
        !manifest_paths.contains(&"foo/Cargo.toml".to_string()),
        "foo/Cargo.toml should NOT be present when preparing for bar"
    );

    // Also verify foo's dependency (either) doesn't affect bar's recipe
    // by checking that foo's manifest contents are not in the recipe
    for manifest in &bar_skeleton.manifests {
        assert!(
            !manifest.contents.contains("foo"),
            "Manifest should not contain foo: {:?}",
